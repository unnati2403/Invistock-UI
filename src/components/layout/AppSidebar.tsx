...
export function AppSidebar() {
  ...
  const renderMenu = (list: any[], depth = 0) => {
    return list.map((item) => {
      ...
      if (isCollapsed) {
            ...
            <SidebarMenuItem key={key}>
              ...
              >
                <PopoverTrigger asChild>
                  ...
                  >
                    <SidebarMenuButton
                      className={`flex items-center justify-center h-11 ${parentHighlightClass}`}
                    >
                ...
                <PopoverContent
                  side="right"
                  align="start"
                  className="p-3 min-w-[220px] border-slate-200 shadow-xl bg-white"
                  onMouseEnter={() => openHover(key)}
                  onMouseLeave={() => closeHoverDelayed(key)}
                >
                  <div className="px-3 py-2 text-sm font-semibold text-slate-800 border-b border-slate-200 mb-2">
                    {item.title}
                  </div>
        ...
        const isCurrentPage = item.url && location.pathname === item.url;
          ...
          <SidebarMenuItem key={key}>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent side="right" className="text-sm bg-slate-800 text-white border-slate-700">{item.title}</TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </SidebarMenuItem>
        );
      ...
      return (
        <SidebarMenuItem key={key}>
          {hasChildren ? (
            <SidebarMenuButton
              className={`h-11 px-3 ${textSize} ${parentHighlightClass} transition-all`}
              onClick={() => toggle(key)}
              asChild={false}
            >
            ...
            <SidebarMenuButton className={`h-11 px-3 ${textSize}`} asChild>
              <NavLink
                to={item.url ?? "#"}
          ...
          {!isCollapsed && hasChildren && (
            <div
              className={`ml-4 overflow-hidden transition-all duration-300 border-l border-slate-200 ${
                opened ? "max-h-[600px] py-2" : "max-h-0"
              }`}
            >
              <div className="ml-3 space-y-1">{renderMenu(item.children, depth + 1)}</div>
            </div>
          )}
        </SidebarMenuItem>
      );
  ...
  return (
    <Sidebar collapsible="icon" className="border-r border-slate-300">
      <SidebarContent className="bg-gradient-to-b from-slate-200 via-slate-100 to-slate-50">
        <div className="flex items-center justify-between px-4 py-4 border-b border-slate-300 bg-slate-200/80">
          <div className="flex items-center gap-3 mr-2">
            <div className="h-10 w-10 rounded-xl bg-gradient-to-br from-blue-600 to-purple-600 flex items-center justify-center text-white shadow-lg shadow-blue-600/30">
              <Package className="h-5 w-5" />
            </div>

            {!isCollapsed && <div className="text-xl font-bold bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">Invistock</div>}
          </div>
          ...
          <Button
            ...
            className="hover:bg-blue-100/50 text-slate-500 hover:text-blue-600"
          >
        ...
        <SidebarGroup className="px-3 py-4">
          <SidebarGroupContent>
            <SidebarMenu className="space-y-1">{renderMenu(MENU)}</SidebarMenu>
          </SidebarGroupContent>
        </SidebarGroup>
      </SidebarContent>
    </Sidebar>
