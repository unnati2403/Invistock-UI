...
import BasicInfoStep from "./bundle-steps/BasicInfoStep";
import BundleItemsStep from "./bundle-steps/BundleItemsStep";
import PricingStep from "./bundle-steps/PricingStep";
import WarehousesStep from "./bundle-steps/WarehousesStep";
import DimensionsStep from "./bundle-steps/DimensionsStep";
import ReviewStep from "./bundle-steps/ReviewStep";
...
const STEPS: StepDefinition[] = [
  ...
  { id: 1, label: "Bundle Items", icon: PackageOpen },
...
const INITIAL_FORM_DATA: BundleFormData = {
  sku: "",
  unit: "",
  name: "",
  barcode: "",
...
interface CreateBundleWizardProps {
  onCancel: () => void;
}

export default function CreateBundleWizard({
  onCancel,
}: CreateBundleWizardProps) {
  const [currentStep, setCurrentStep] = useState(0);
  const [completedSteps, setCompletedSteps] = useState<Set<number>>(
    new Set()
  );
  const [formData, setFormData] =
    useState<BundleFormData>(INITIAL_FORM_DATA);
  ...
  const renderStepContent = () => {
    const stepProps: BundleStepProps = {
      formData,
      setFormData,
      pricingMode,
    };
    ...
    switch (currentStep) {
      ...
      case 1:
        return <BundleItemsStep {...stepProps} />;
      case 2:
      ...
      <div className="flex items-start sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-slate-500 text-sm mt-1">
            Combine multiple items into a single sellable bundle.
          </p>
        </div>
        <Button
          variant="outline"
